...
ui <- dashboardPage(
  dashboardHeader(
    ...
    titleWidth = 450 # Adjust this width based on your needs
  ),
      ...
      tabItem(tabName = "results",
              fluidPage(
                ...
                div(
                  ...
                  selectInput("fileToView", "View Result files", choices = c("charact_x.csv", "daily_charact_x.csv", "trials_x.csv"))
                ),
                DTOutput("viewData")
              )
      ),
...
server <- function(input, output, session) {
  ...
  observe({
    req(analysisDone())
    tryCatch({
      source(paste0(codesPath,"/vizualization.R"))
    }, error = function(e) {
