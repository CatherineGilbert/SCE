...
For the sake of reproducibility, the SCE app is bundled with a Docker file and the files necessary to build the working environment. "renv.lock" contains the versioned R packages; the folder "next_gen_apsim" contains the files for the Next-Gen APSIM distribution. In the case that packages and software become outdated, building from the Docker file allows you to replicate the app in its original environment.
...
4. [Download and install a copy of RTools,](https://cran.r-project.org/bin/windows/Rtools/) if you don't have one.
5.  Open R or your R IDE. From the R console, run the command `setwd()` and set the working directory to the path of the extracted SCE folder. Ex: `setwd("example/path/to/SCE-main")`.
6.  Run the command `shiny::runApp()`. The application should open.
...
Under "Select Maturity Handling", the user can choose which system the script should use to translate the "Genetics" information in the input file into cultivar maturities. This in turn determines the cultivar parameter files that the APSIM simulations use to set the phenology of each trial. The three options for maturity handling are "Maize RM", "Soybean RM", and "Use given APSIM cultivars". The first option is intended to be used with the "Maize_Template" model, the second with the "Soy_Template" model, and the last option with any other custom model.

### 3.3.1. Maize Maturity Handling

For the "Maize RM" option, the "Genetics" values of the input file should be taken from the cultivar RM and formatted as character string of A or B and a number of growing degree days (ex: "B_100", "A120", "B 95", etc., are all acceptable). Maize maturity is handled through binning: the RM values of the inputs are matched to their closest available maturities in the generic Maize A and B cultivars.

### 3.3.2. Soy Maturity Handling

For the "Soybean RM" option, the "Genetics" values of the input file should be formatted as a decimal RM where the floor of the value corresponds to the cultivar's maturity group classification. RM values 0 -- 0.999, 1 -- 1.999, 2 -- 2.999, etc., translate to maturity groups "0", "I", "II", etc. The tool supports maturities 000 -- X, with 000 available as RM values -2 -- -1.001 and 00 available as values -1 -- -0.001.
...
The early and late variants of each cultivar were created by linear interpolation from the coefficients of neighboring Generic APSIM cultivars.

### 3.3.3. Custom Maturity Handling

For the "Use given APSIM cultivars" maturity handling option, the "Genetics" values of the input file should be the exact names of the APSIM cultivars which the user would like to use for the trial simulation.
...
::: {style="text-align: center; font-size: 90%; margin-top: 0.5em; margin-bottom: 1.5em;"}
<a href="pictures/fig6b_colored_combined.png" target="_blank">
  <img src="pictures/fig6b_colored_combined.png" style="width: 100%; cursor: zoom-in;" />
</a>
...
"Maturity Handling" is the system that the app uses to set the crop phenology. This should match the crop type you're using. Choose "Maize" for maturity handling.
...
-   "Genetics" refers to the cultivar maturity designation. 
    ...
    -   When using the "Use given APSIM cultivars" maturity handling, or working with any crop besides soybean and maize, Genetics is the exact name of the APSIM cultivar that will be referenced for the developmental timing. 
...
Example input files are available in the "example_input_files" folder, and can be downloaded from the front page of the app.

### 8.2.1. Example One

Let's try building an input file. Our first example will use data from the Northern Region Uniform Soybean Tests, a multi-environment trial of soybean varieties in North America. This is from the same larger dataset which was used to build the soybean example file. The specific data we'll be using is available through the SoyURT package.

Source: Using soybean historical field trial data to study genotype by environment variation and identify mega-environments with the integration of genetic and non-genetic factors. Matheus D Krause, Kaio O G Dias, Asheesh K Singh, William D Beavis. bioRxiv 2022.04.11.487885; doi: <https://doi.org/10.1101/2022.04.11.487885>
...
soy.input <- rename(pheno, Site = location, Longitude = longitude, Latitude = latitude) 
```

First we assess the data to see what information we're missing for the input file. Thankfully, we seem to have everything already. The "Site", "Latitude", and "Longitude" columns can be obtained by simply renaming two existing columns, while "Genetics" and "Planting" can be derived from "maturity_group" and "average_planting_date" / "year" respectively.

```{r}
soy.input <- mutate(soy.input, Genetics = maturity_group + 0.5) # We'll use the maturity group of the test as a proxy for genetic maturity of these cultivars, since we don't have more information on them. We add 0.5 to set the genetic maturity within the middle of that maturity group, as opposed to an early or late variant. 2.5 = "a standard MG II", 3.5 = "a standard MG III".

soy.input <- mutate(soy.input, Planting = as.character(as.Date(average_planting_date, format = c("%m/%d/%y")))) # Convert planting dates to the appropriate format. 

soy.input <- mutate(soy.input, Planting = ifelse(is.na(Planting), year, Planting)) # Some entries only have a year, and no planting date. For these, we will assign "Planting" as the year they were planted, and leave the planting date up to the simulation's determination. 
```

We subset to the necessary columns to get a valid input file.

```{r}
soy.input <- select(soy.input, Site, Latitude, Longitude, Planting, Genetics) 

glimpse(soy.input)
```

Our input file here has 39006 rows, or 39006 sets of trial parameters. That's a lot. Thankfully, we only need to run the analysis for unique combinations of starting parameters.

```{r}
soy.input_full <- soy.input # We can save a full version of the input, and use it later to join our final results back to our original data. The indices of this copy match the indices of the original data. 

soy.input <- distinct(soy.input) # Filter to distinct combinations of trial parameters.

head(soy.input)
```

Our final input file is 1591 rows long. The SCE will generate 1591 unique seasonal profiles for this data.

### 8.2.2. Example Two

Let's try a second example. We will create a custom input file using datasets from a multi-environment trial of maize conducted in Yunnan, China. This data is available through the agridat package.

Source: Fan, X.M. and Kang, M.S. and Chen, H. and Zhang, Y. and Tan, J. and Xu, C. (2007). Yield stability of maize hybrids evaluated in multi-environment trials in Yunnan, China. Agronomy Journal, 99, 220-228. <https://doi.org/10.2134/agronj2006.0144>

```{r}
library(agridat)

fan.input <- fan.stability
glimpse(fan.stability)
```

We need "Site", "Latitude", "Longitude", "Genetics," and "Planting". Looking at this data, we could derive Site from "loc", Genetics from "mat", and Planting from "year". Unfortunately, this dataset doesn't have the site coordinates. Fortunately, that data is available in the paper's supplementary material-- which also includes exact planting dates.

```{r}
fan.supplement <- read_csv("fan.supplement.csv")
glimpse(fan.supplement)
```

We need to do some manipulation to get this data into the correct formatting. We'll then join the main and supplementary datasets together.

```{r}
#Coordinates should be in decimal format. Convert them. 
degmin_to_decimal <- function(x) {
  x <- gsub("[^0-9.°-]", "", x)  
  parts <- strsplit(x, "°")
...
fan.supplement <- mutate(fan.supplement, Latitude = degmin_to_decimal(Latitude), 
       Longitude = degmin_to_decimal(Longitude))

#Convert the planting dates to the YYYY-MM-DD format. 
fan.supplement <- mutate(fan.supplement, Planting = as.character(as.Date(`Planting date`, format = c("%d/%m/%Y"))))

#Need to join planting dates from the supplementary material to the matching years in the main dataset. Create a year column for joining.
fan.supplement <- mutate(fan.supplement, year = as.numeric(str_sub(Planting, start = 1, end = 4)))

#On the side of the original dataset, we need to rename two columns.
fan.input <- rename(fan.input, Site = loc, Genetics = maturity)

#.. and convert Genetics to the appropriate format.
fan.input <- mutate(fan.input, Genetics = paste0(Genetics,"A")) 

#Rename the location column for joining. 
fan.supplement <- rename(fan.supplement, Site = Location)

#Join the datasets together. 
fan.input <- left_join(fan.input, fan.supplement) 

glimpse(fan.input)
```

We subset the data to the columns that we'll use to build the input file.

```{r}
fan.input <- select(fan.input, Site, Latitude, Longitude, Planting, Genetics)

glimpse(fan.input)
```

We can save a copy of this data as the full version of the input which matches the indices of the original file (fan.stability). We can then use a version of the input with only unique combinations of our trial parameters as the actual input to the analysis. This prevents redundant simulations and greatly saves on analysis time.

```{r}
fan.input_full <- fan.input
fan.input <- distinct(fan.input)

head(fan.input)
```

## 8.3 Creating a Custom Template Model

You can generally create a custom template file for maize / soybean by editing any of the template files provided with the SCE and saving it as a new .apsimx.

The parameters of the simulation scripts and modules can be freely edited, which is helpful for customizing a simulation to more closely resemble the known conditions of an actual trial. (Ex: modifying fertilizer inputs, tillage, row spacing, etc.) Adding your own manager scripts is also possible, although their compatibility isn't guaranteed. At minimum, the Sowing and ReaperMan manager scripts from the template files need to maintain their paths and reporting variables so the analysis can set and retrieve simulation timing.

Because this is an APSIM file, you can also customize the outputs of the simulations through the Reporting Variables module. This allows you to create a seasonal covariate, reported at each stage of development, for practically any parameter within the APSIM model. At minimum, Date, DOY, Stage, SimSowDate, SimMatDate, SimHarvestDate, Rain, ThermalTime, AccEmTT, and Result need to be included in the template's reporting variables or else later analysis will break.

### 8.3.1 Creating a Template Model for a New Crop

Open one of the template models provided with the tool, either "Soybean_Template.apsimx" or "Maize_Template.apsimx". Save it under a new name. This will be the basis of your new crop model.

From the front page of APSIM, click "Open an Example" and open the example for the crop of your choosing. Copy and paste the internal crop model from the example file (ex: "Sorghum", "Rice", "Wheat") into the Field area of your new model. You may delete the old crop model ("Soybean" or "Maize").

In the Sowing module, set Crop to the new crop. In the Fertilize at sowing module, set Crop to be fertilized to the new crop.

You may adjust any other of the model's parameters as you'd like. Save the new crop model.

### 8.3.2. Sorghum Demonstration

In the "SCE/www" folder, we've saved a template model for Sorghum that was created according to the procedure above. The input file below can be used to test and run it.

```{r}
#Creating an example input file for Sorghum
sorg.input <- tibble(
           Site = c("Melkassa","Mieso","Kobo","Melkassa","Mieso","Kobo"), 
           Latitude = c(8.4,8.683,12.15,8.4,8.683,12.15),
           Longitude = c(39.35,39.367,39.6167,39.35,39.367,39.6167),
           Genetics = c("early","medium","late","medium","medium","early"), #names of cultivars included in the APSIM "Sorghum" module. These cultivar descriptions tell the system how the crops will mature. 
           Planting = c("2020-05-05","2020-05-06","2020-05-07","2020-05-05","2020-05-06","2020-05-07")
           )

write_csv(sorg.input,"sorg.input.csv")
```

**When running an analysis with a template model for a new crop, we must set the Maturity Handling to the final option** so that the analysis parses the Genetics information as the names of the APSIM cultivars you will be using to define the crop's phenology.
